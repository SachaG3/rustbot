...
pub async fn jeux(ctx: &Context, msg: &Message) -> CommandResult {
    if let Err(why) = msg
        .channel_id
        .say(&ctx.http, "https://discord.gg/jszvZm36r8")
        .await
    {
        error!("Erreur lors de l'envoi du message: {:?}", why);
    }
    ...
    match get_user_by_discord_id(&pool, msg.author.id.0).await {
        ...
        Ok(None) => {
            if let Err(why) = msg
                ...
                .say(
                    &ctx.http,
                    "Vous devez d'abord créer un profil avec la commande ^^NP",
                )
                .await
            {
                error!("Erreur lors de l'envoi du message: {:?}", why);
        ...
        Err(e) => {
            ...
            if let Err(why) = msg
                ...
                .say(
                    &ctx.http,
                    "Une erreur est survenue lors de la recherche de votre profil.",
                )
                .await
            {
                error!("Erreur lors de l'envoi du message: {:?}", why);
